...
mod tests {
    ...
    #[tokio::test]
    async fn test_api_create_session() -> eyre::Result<()> {
        // Arrange
        ...
        println!(
            "  Transaction receipt: {}",
            second_session_result.transaction_receipt_json
        );
        ...
        let expected_second_session_hash: FixedBytes<32> = {
            let expected_second_session_hash = hex::decode(
                "0x86c2b7aabf2f8c9cf63dee08c5dbdc102bb1dae07512184f2d538e4dba6c50fc",
            )?;
            FixedBytes::from_slice(&expected_second_session_hash)
        };
        eyre::ensure!(
            second_session_hash.fixed_bytes() == expected_second_session_hash,
            "Second session hash does not match expected value"
        );
