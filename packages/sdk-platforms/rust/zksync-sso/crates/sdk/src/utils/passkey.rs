...
pub fn passkey_hash_signature_response_format(
    ...
    let full_formatted_sig_hex = alloy::hex::encode(full_formatted_sig);
...
pub fn passkey_hash_signature_response_format_bytes(
    ...
    debug!(
        "XDB - utils::passkey::passkey_hash_signature_response_format - passkey_response: {passkey_response:?}"
    );
    debug!(
        "XDB - utils::passkey::passkey_hash_signature_response_format - contracts: {contracts:?}"
    );
